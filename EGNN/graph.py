...
class Graph:
    ...
    def get_total_size(self):
        return (
            self.get_num_nodes() * self.get_node_nf() + self.get_num_edges() * self.get_edges_nf()
        )
    ...
    def get_dense_graph(self, store=True, loops=False):
        if self.adjacency is None or self.edges_dense is None or self.edge_attr_dense is None:
            ...
            if store:
                self.edges_dense, self.edge_attr_dense, self.adjacency = (
                    edges_dense,
                    edge_attr_dense,
                    adjacency,
                )
...
def dense2networkx(adj_dense):
    g = nx.from_numpy_array(adj_dense.detach().to("cpu").numpy(), create_using=nx.Graph)
    return g
