...
DEFAULT_DATADIR = os.environ.get(
    "QM9_DATADIR", os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data"))
)
...
def setup_shared_args(parser):
    ...
    parser.add_argument("--shuffle", action=BoolArg, default=True, help="Shuffle minibatches.")
    parser.add_argument(
        "--seed",
        type=int,
        default=1,
    ...
    parser.add_argument("--log-level", type=str, default="info", help="Logging level to output")
    ...
    parser.add_argument(
        "--predict", action=BoolArg, default=True, help="Save predictions. (default)"
    )
    ...
    # Computation options
    parser.add_argument("--cuda", dest="cuda", action="store_true", help="Use CUDA (default)")
    parser.add_argument("--no-cuda", "--cpu", dest="cuda", action="store_false", help="Use CPU")
    parser.set_defaults(cuda=True)

    parser.add_argument(
        "--float", dest="dtype", action="store_const", const="float", help="Use floats."
    )
    parser.add_argument(
        "--double", dest="dtype", action="store_const", const="double", help="Use doubles."
    )
    ...
    # Model options
    parser.add_argument(
        "--num-cg-levels", type=int, default=4, metavar="N", help="Number of CG levels (default: 4)"
    )
    ...
    parser.add_argument(
        "--gaussian-mask", action="store_true", help="Use gaussian mask instead of sigmoid mask."
    )

    parser.add_argument(
        "--edge-cat",
        action="store_true",
        help="Concatenate scalars from different ell values in the edge network.",
    )
    parser.add_argument(
        "--target",
        type=str,
