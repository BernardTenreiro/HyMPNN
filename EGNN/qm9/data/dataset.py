    ...
    def __init__(
        ...
        if subtract_thermo:
            ...
            if len(thermo_targets) == 0:
                logging.warning(
                    "No thermochemical targets included! Try reprocessing dataset with --force-download!"
                )
            else:
                logging.info(
                    "Removing thermochemical energy from targets {}".format(
                        " ".join(thermo_targets)
                    )
                )
        ...
        self.data["one_hot"] = self.data["charges"].unsqueeze(-1) == included_species.unsqueeze(
            0
        ).unsqueeze(0)
