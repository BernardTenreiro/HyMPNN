...
def initialize_datasets(
    args, datadir, dataset, subset=None, splits=None, force_download=False, subtract_thermo=False
):
    """
    Initialize datasets.
...
def _get_species(datasets, ignore_check=False):
    ...
    all_species = torch.cat([dataset["charges"].unique() for dataset in datasets.values()]).unique(
        sorted=True
    )

    # Find the unique list of species in each dataset.
    split_species = {
        split: species["charges"].unique(sorted=True) for split, species in datasets.items()
    }
