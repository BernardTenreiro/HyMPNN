...
// models by the same code path, so it does not bias the EGNN/HyEGNN comparison.
//
// Reference (models_pairwise.py::E_GCL_mask.forward -> models/gcl.py::E_GCL):
//     radial = |x_i - x_j|^2
...
__global__ void epilogue_fwd(
    const float* __restrict__ h, const float* __restrict__ m,
    const float* __restrict__ att, const float* __restrict__ emask,
    const long* __restrict__ off,
    float* __restrict__ B, int N, int nf, int has_mask) {
  int n = blockIdx.x;
  if (n >= N) return;
  const int Bw = 2*nf;
...
__global__ void epilogue_bwd(
    const float* __restrict__ m, const float* __restrict__ att,
    const float* __restrict__ emask, const float* __restrict__ dB,
    const long* __restrict__ row,
    float* __restrict__ dh, float* __restrict__ dm, float* __restrict__ datt,
    int E, int nf, int has_mask, int N) {
  int e = blockIdx.x;
  if (e >= E) return;
  long n = row[e];
  const int Bw = 2*nf;
...
__global__ void epilogue_bwd_identity(
    const float* __restrict__ dB, float* __restrict__ dh, int N, int nf) {
  int n = blockIdx.x;
  if (n >= N) return;
  const int Bw = 2*nf;
  for (int k = threadIdx.x; k < nf; k += blockDim.x)
...
inline int threads_for(int nf) { return nf <= 64 ? 64 : 128; }

} // namespace

torch::Tensor dense_prologue_forward(
    torch::Tensor h, torch::Tensor x, torch::Tensor row, torch::Tensor col) {
  ...
  if (E > 0)
    prologue_fwd<<<E, threads_for(nf), 0, stream>>>(
        ...
        row.data_ptr<long>(), col.data_ptr<long>(),
        A.data_ptr<float>(), E, nf);
  return A;
}

std::vector<torch::Tensor> dense_prologue_backward(
    torch::Tensor h, torch::Tensor x, torch::Tensor row, torch::Tensor col,
    torch::Tensor dA) {
  int E = row.size(0), nf = h.size(1);
  ...
  if (E > 0)
    prologue_bwd<<<E, threads_for(nf), 0, stream>>>(
        ...
        dA.contiguous().data_ptr<float>(),
        dh.data_ptr<float>(), dx.data_ptr<float>(), E, nf);
  return {dh, dx};
}

torch::Tensor dense_epilogue_forward(
    torch::Tensor h, torch::Tensor m, torch::Tensor att,
  ...
  epilogue_fwd<<<N, threads_for(nf), 0, stream>>>(
      ...
      off.data_ptr<long>(), B.data_ptr<float>(), N, nf, has_mask);
  return B;
}

std::vector<torch::Tensor> dense_epilogue_backward(
    torch::Tensor h, torch::Tensor m, torch::Tensor att, torch::Tensor emask,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  epilogue_bwd_identity<<<N, t, 0, stream>>>(
      dB.contiguous().data_ptr<float>(), dh.data_ptr<float>(), N, nf);
  if (E > 0)
    epilogue_bwd<<<E, t, t*sizeof(float), stream>>>(
        m.contiguous().data_ptr<float>(), att.contiguous().data_ptr<float>(),
        has_mask ? emask.contiguous().data_ptr<float>() : nullptr,
        dB.contiguous().data_ptr<float>(), row.data_ptr<long>(),
        dh.data_ptr<float>(), dm.data_ptr<float>(), datt.data_ptr<float>(),
        E, nf, has_mask, N);
  return {dh, dm, datt};
}
