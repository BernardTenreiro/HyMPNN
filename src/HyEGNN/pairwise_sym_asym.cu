...
__global__ void prologue_fwd(
  ...
  if (threadIdx.x == 0) { A[(long)e*Aw + 2*nf] = rad; B[(long)e*Bw + nf] = rad; }
}
...
__global__ void epilogue_fwd(
    const float* __restrict__ h, const float* __restrict__ z_s,
...
__global__ void epilogue_bwd(
    ...
    const long* __restrict__ rows, const long* __restrict__ cols,
    float* __restrict__ dh, float* __restrict__ dz_s, float* __restrict__ dgate,
    int E, int nf, int eager_compat) {
  int e = blockIdx.x;
  if (e >= E) return;
  long ri = rows[e], ci = cols[e];
  for (int k = threadIdx.x; k < nf; k += blockDim.x) {
...
__global__ void zero_pair_rows(float* __restrict__ dh,
...
inline int threads_for(int nf) { return nf < 128 ? 64 : 128; }

} // namespace

std::vector<torch::Tensor> prologue_forward(
...
std::vector<torch::Tensor> prologue_backward(
    torch::Tensor h, torch::Tensor x, torch::Tensor rows, torch::Tensor cols,
...
torch::Tensor epilogue_forward(
    torch::Tensor h, torch::Tensor z_s, torch::Tensor gate,
  ...
  if (E > 0)
    epilogue_fwd<<<E, threads_for(nf), 0, stream>>>(
        ...
        gate.contiguous().data_ptr<float>(),
        rows.data_ptr<long>(), cols.data_ptr<long>(),
        out.data_ptr<float>(), E, nf);
  return out;
}

std::vector<torch::Tensor> epilogue_backward(
    torch::Tensor h, torch::Tensor gate, torch::Tensor dout,
    torch::Tensor rows, torch::Tensor cols, bool eager_compat) {
  int E = rows.size(0), nf = h.size(1);
  ...
  if (E > 0) {
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    zero_pair_rows<<<E, t, 0, stream>>>(dh.data_ptr<float>(),
        rows.data_ptr<long>(), cols.data_ptr<long>(), E, nf);
    epilogue_bwd<<<E, t, 0, stream>>>(
        h.data_ptr<float>(), gate.contiguous().data_ptr<float>(),
        dout.contiguous().data_ptr<float>(),
        rows.data_ptr<long>(), cols.data_ptr<long>(),
        dh.data_ptr<float>(), dz_s.data_ptr<float>(), dgate.data_ptr<float>(),
        E, nf, eager_compat ? 1 : 0);
  }
