"""Fully fused PairwiseSymAsymLayer (nf=64): one kernel per direction with the
MLP weights resident in shared memory. See pairwise_fused_mlp.cu for why.
...
def load_kernels(verbose=False):
    ...
    if _K is None:
        ...
        _K = load(name="hyegnn_pairwise_mlp",
                  sources=[os.path.join(here, "pairwise_fused_mlp.cu")],
...
class _Fused(torch.autograd.Function):
    ...
    def forward(ctx, h, x, rows, cols, W1s, b1s, W2s, b2s, W1g, b1g, W2g, b2g):
        ...
        ctx.save_for_backward(h, x, rows, cols, W1s, b1s, W2s, b2s, W1g, b1g, W2g, b2g,
                              sA, sS1, sHS, sG1, sHG, sGate)
        return out

    @staticmethod
    def backward(ctx, dout):
        (h, x, rows, cols, W1s, b1s, W2s, b2s, W1g, b1g, W2g, b2g,
         sA, sS1, sHS, sG1, sHG, sGate) = ctx.saved_tensors
        dh, dx, dS1, dZ, dG1, dG2 = load_kernels().fused_backward(
            h, x, rows, cols, W1s, b1s, W2s, b2s, W1g, b1g, W2g, b2g,
            dout, sS1, sG1, sGate)
        nf = h.size(1)
        # weight grads: reductions over pairs -> GEMMs (E is small, these are tiny)
        dW1s = dS1.t() @ sA;            db1s = dS1.sum(0)
        dW2s = dZ.t()  @ sHS;           db2s = dZ.sum(0)
        dW1g = dG1.t() @ sA[:, nf:];    db1g = dG1.sum(0)
        dW2g = dG2.t() @ sHG;           db2g = dG2.sum(0)
        return (dh, dx, None, None, dW1s, db1s, dW2s, db2s, dW1g, db1g, dW2g, db2g)


class FusedPairwiseSymAsymMLP(nn.Module):
    """Drop-in for PairwiseSymAsymLayer with hidden_nf == 64."""

    def __init__(self, hidden_nf, act_fn=nn.SiLU()):
        super().__init__()
        assert hidden_nf == 64, "fully fused kernel is specialised for nf=64"
        self.hidden_nf = hidden_nf
        self.f_s = nn.Sequential(nn.Linear(2*hidden_nf + 1, hidden_nf), act_fn,
                                 nn.Linear(hidden_nf, hidden_nf))
        self.f_d_gate = nn.Sequential(nn.Linear(hidden_nf + 1, hidden_nf), act_fn,
    ...
    def forward(self, h, x, rows, cols):
        if rows.size(0) == 0:
            return h
        return _Fused.apply(h, x, rows, cols,
                            self.f_s[0].weight, self.f_s[0].bias,
                            self.f_s[2].weight, self.f_s[2].bias,
                            self.f_d_gate[0].weight, self.f_d_gate[0].bias,
                            self.f_d_gate[2].weight, self.f_d_gate[2].bias)
