// Fully fused symmetric/asymmetric layer: gather + MLPs + gate + scatter in one
...
__device__ __forceinline__ void load_weights(float *sm, const float *W1s, const float *b1s,
                                             const float *W2s, const float *b2s, const float *W1g,
                                             const float *b1g, const float *W2g, const float *b2g) {
    for (int i = threadIdx.x; i < NF * A_W; i += blockDim.x)
        sm[OFF_W1S + i] = W1s[i];
    for (int i = threadIdx.x; i < NF * NF; i += blockDim.x) {
        sm[OFF_W2S + i] = W2s[i];
        sm[OFF_W2G + i] = W2g[i];
    }
    for (int i = threadIdx.x; i < NF * B_W; i += blockDim.x)
        sm[OFF_W1G + i] = W1g[i];
    if (threadIdx.x < NF) {
        sm[OFF_B1S + threadIdx.x] = b1s[threadIdx.x];
        sm[OFF_B2S + threadIdx.x] = b2s[threadIdx.x];
        sm[OFF_B1G + threadIdx.x] = b1g[threadIdx.x];
        sm[OFF_B2G + threadIdx.x] = b2g[threadIdx.x];
    }
}

// ------------------------------------------------------------------ forward
__global__ void fused_fwd(const float *__restrict__ h, const float *__restrict__ x,
                          ...
                          float *__restrict__ out, float *__restrict__ sA, float *__restrict__ sS1,
                          float *__restrict__ sHS, float *__restrict__ sG1, float *__restrict__ sHG,
                          float *__restrict__ sGate, int E) {
    ...
    for (int p = 0; p < PAIRS_PER_BLOCK; ++p) {
        ...
        A[NF + k] = fabsf(hd);
        if (k == 0) {
            float d0 = x[ri * 3] - x[ci * 3], d1 = x[ri * 3 + 1] - x[ci * 3 + 1],
                  d2 = x[ri * 3 + 2] - x[ci * 3 + 2];
...
#pragma unroll 8
        for (int i = 0; i < NF; ++i)
            z += sm[OFF_W2S + k * NF + i] * V0[i];
        float g2 = sm[OFF_B2G + k];
#pragma unroll 8
        for (int i = 0; i < NF; ++i)
            g2 += sm[OFF_W2G + k * NF + i] * V1[i];
        ...
        sG1[(long)e * NF + k] = g1;
        sHG[(long)e * NF + k] = hg;
        sGate[(long)e * NF + k] = gate;
        for (int i = k; i < A_W; i += NF)
            sA[(long)e * A_W + i] = A[i];
...
__global__ void fused_bwd(const float *__restrict__ h, const float *__restrict__ x,
                          const long *__restrict__ rows, const long *__restrict__ cols,
                          const float *W1s, const float *b1s, const float *W2s, const float *b2s,
                          const float *W1g, const float *b1g, const float *W2g, const float *b2g,
                          const float *__restrict__ dout, const float *__restrict__ sS1,
                          const float *__restrict__ sG1, const float *__restrict__ sGate,
                          float *__restrict__ dh, float *__restrict__ dx, float *__restrict__ dS1,
                          float *__restrict__ dZ, float *__restrict__ dG1, float *__restrict__ dG2,
                          int E) {
    extern __shared__ float sm[];
    load_weights(sm, W1s, b1s, W2s, b2s, W1g, b1g, W2g, b2g);
    __syncthreads();
...
#pragma unroll 8
        for (int o = 0; o < NF; ++o) {
            dhs += sm[OFF_W2S + o * NF + k] * V0[o];
            dhg += sm[OFF_W2G + o * NF + k] * V1[o];
        }
...
#pragma unroll 8
        ...
        if (k == 0) {
            float dr = 0.f;
            for (int o = 0; o < NF; ++o)
                dr += sm[OFF_W1S + o * A_W + 2 * NF] * V0[o] + sm[OFF_W1G + o * B_W + NF] * V1[o];
            A[2 * NF] = dr;
        }
...
std::vector<torch::Tensor> fused_forward(torch::Tensor h, torch::Tensor x, torch::Tensor rows,
                                         torch::Tensor cols, torch::Tensor W1s, torch::Tensor b1s,
                                         torch::Tensor W2s, torch::Tensor b2s, torch::Tensor W1g,
                                         torch::Tensor b1g, torch::Tensor W2g, torch::Tensor b2g) {
    TORCH_CHECK(h.size(1) == NF, "fused pairwise MLP kernel requires hidden_nf == 64");
    ensure_smem();
    const int E = rows.size(0);
    auto out = h.clone();
    auto opt = h.options();
    auto sA = torch::empty({E, A_W}, opt), sS1 = torch::empty({E, NF}, opt),
         sHS = torch::empty({E, NF}, opt), sG1 = torch::empty({E, NF}, opt),
         sHG = torch::empty({E, NF}, opt), sGate = torch::empty({E, NF}, opt);
    if (E > 0) {
        ...
        fused_fwd<<<blocks, NF, SMEM_BYTES, stream>>>(
            ...
            rows.data_ptr<long>(), cols.data_ptr<long>(), W1s.contiguous().data_ptr<float>(),
            b1s.contiguous().data_ptr<float>(), W2s.contiguous().data_ptr<float>(),
            b2s.contiguous().data_ptr<float>(), W1g.contiguous().data_ptr<float>(),
            b1g.contiguous().data_ptr<float>(), W2g.contiguous().data_ptr<float>(),
            b2g.contiguous().data_ptr<float>(), out.data_ptr<float>(), sA.data_ptr<float>(),
            sS1.data_ptr<float>(), sHS.data_ptr<float>(), sG1.data_ptr<float>(),
            sHG.data_ptr<float>(), sGate.data_ptr<float>(), E);
    }
    return {out, sA, sS1, sHS, sG1, sHG, sGate};
}

std::vector<torch::Tensor> fused_backward(torch::Tensor h, torch::Tensor x, torch::Tensor rows,
                                          torch::Tensor cols, torch::Tensor W1s, torch::Tensor b1s,
                                          torch::Tensor W2s, torch::Tensor b2s, torch::Tensor W1g,
                                          torch::Tensor b1g, torch::Tensor W2g, torch::Tensor b2g,
                                          torch::Tensor dout, torch::Tensor sS1, torch::Tensor sG1,
                                          torch::Tensor sGate) {
    ensure_smem();
    const int E = rows.size(0);
    ...
    auto opt = h.options();
    auto dS1 = torch::empty({E, NF}, opt), dZ = torch::empty({E, NF}, opt),
         dG1 = torch::empty({E, NF}, opt), dG2 = torch::empty({E, NF}, opt);
    if (E > 0) {
        const int blocks = (E + PAIRS_PER_BLOCK - 1) / PAIRS_PER_BLOCK;
        cudaStream_t stream = at::cuda::getCurrentCUDAStream();
        fused_bwd<<<blocks, NF, SMEM_BYTES, stream>>>(
            ...
            b2s.contiguous().data_ptr<float>(), W1g.contiguous().data_ptr<float>(),
            b1g.contiguous().data_ptr<float>(), W2g.contiguous().data_ptr<float>(),
            b2g.contiguous().data_ptr<float>(), dout.contiguous().data_ptr<float>(),
            sS1.data_ptr<float>(), sG1.data_ptr<float>(), sGate.data_ptr<float>(),
            dh.data_ptr<float>(), dx.data_ptr<float>(), dS1.data_ptr<float>(), dZ.data_ptr<float>(),
            dG1.data_ptr<float>(), dG2.data_ptr<float>(), E);
    }
