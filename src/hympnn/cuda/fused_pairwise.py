"""CUDA-fused symmetric/asymmetric pairwise layer.
...
def load_kernels(verbose=False):
    ...
    if _KERNELS is None:
        ...
        _KERNELS = load(
            name="hympnn_pairwise",
            sources=[os.path.join(here, "pairwise_symmetric_asymmetric.cu")],
...
class FusedSymmetricAsymmetricPairwiseLayer(nn.Module):
    """Fused symmetric/asymmetric pairwise update layer."""
    ...
    def load_from(self, eager_layer):
        """Copy weights from an eager symmetric/asymmetric pairwise layer."""
