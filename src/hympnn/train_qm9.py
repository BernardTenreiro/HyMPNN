...
from .cuda.graphs import (
    BucketedCudaGraphRunner,
    replace_linear_layers_for_graph_capture,
)
from .data.qm9 import features as qm9_features
from .data.qm9 import loaders as qm9_loaders
from .data.qm9.collate import collate_batch
from .data.qm9.edge_scheduling import (
    SparseEdgeCollator,
...
PROFILE_BATCH_COUNT = int(os.environ.get("HYMPNN_PROFILE", os.environ.get("EGNN_PROFILE", "0")))
...
def install_fused_layers(args: argparse.Namespace, model_info: ModelInfo) -> None:
    ...
    if args.fused_pairwise:
        ...
        fused_class = (
            FusedSymmetricAsymmetricPairwiseMLP
            if model_info.pairwise_features == 64
            else FusedSymmetricAsymmetricPairwiseLayer
        )
...
def run_epoch(
    ...
    for batch_index, batch in enumerate(loader):
        ...
        nodes = qm9_features.build_node_features(
...
def run_experiment(
    ...
    if args.train_fraction < 1:
        ...
        qm9_loaders.shutdown_dataloaders({"train": original_loader})
        dataloaders["train"] = qm9_loaders.create_dataloader(
    ...
    if model_info.uses_sparse_edges:
        ...
        if args.num_workers > 0:
            ...
            rebuilt_dataloaders = qm9_loaders.rebuild_dataloaders(
                dataloaders,
                args.batch_size,
                args.num_workers,
                SparseEdgeCollator(
...
def main() -> None:
    ...
    try:
        ...
        qm9_loaders.shutdown_dataloaders(dataloaders)
